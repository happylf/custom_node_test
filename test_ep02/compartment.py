...
class PersonMoving:
    ...
    def INPUT_TYPES(s):        
        return {
            "required": {
                ...
                "sampler_name": (comfy.samplers.KSampler.SAMPLERS, ),
                "scheduler": (comfy.samplers.KSampler.SCHEDULERS, ),     
                "x": ("INT", {"default": 0, "min": 0, "max": 100000}),
                "y": ("INT", {"default": 0, "min": 0, "max": 100000}),
                "r": ("INT", {"default": 0, "min": 0, "max": 100000}),                
                "threshold": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01}),                      
                "width": ("INT", {"default": 512, "min": 16, "max": MAX_RESOLUTION, "step": 8}),
                "height": ("INT", {"default": 512, "min": 16, "max": MAX_RESOLUTION, "step": 8}),
                "batch_size": ("INT", {"default": 1, "min": 1, "max": 4096}),
    ...
    def todo(self, ckpt_name, in_posi, in_nega, CN_img, CN_name, IPAd_img, IPAd_name, backg_img, 
             seed, steps, cfg, sampler_name, scheduler, x, y, r, threshold, width, height, batch_size):  
        # sampler     
        set_IPAdapter = IPAdapter_set(IPAd_img, IPAd_name)
        motion_model = "None"
        pre_sampler = PreSampler_CN().todo(ckpt_name, in_posi, in_nega, CN_name, motion_model, 
            r, r, batch_size, CN_img, set_IPAdapter)[0]
        p_img, _ = SamplerInsp(pre_sampler, seed, steps, cfg, sampler_name, scheduler, denoise=1.0)
        p_img = Reactor_apply(p_img)

        # ImageCompositeMasked
        mask_img, _, _ = yolo_detect(p_img, threshold)
        b_img = img_scale_adjust(backg_img, width, height)
        resize_source = False
        image = nodes_mask.ImageCompositeMasked().composite(b_img, p_img, x, y, resize_source, mask_img)[0]

        return image, p_img, mask_img
